...
export default function ReligionsTab({ schoolId }: { schoolId: string }) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) {
        ...
        const { error } = await supabase.from("school_religions").insert(payload);
        if (error) throw error;
  ...
  async function handleDelete() {
    if (!deleteId) return;
    const { error } = await supabase.from("school_religions").delete().eq("id", deleteId);
    if (error) toast.error(error.message);
    else { toast.success("Religion deleted"); fetchData(); }
    setDeleteId(null);
  }

  async function toggleActive(item: Religion) {
    const { error } = await supabase
      .from("school_religions")
    ...
    <div className="rounded-xl border bg-card overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b bg-muted/30">
        <div>
          ...
          <p className="text-xs text-muted-foreground">e.g. Christianity, Islam, Others</p>
        </div>
        ...
        <DialogContent>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div>
              ...
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Christianity" required />
            </div>
            <div>
              <label className="text-sm font-medium block mb-1">Code</label>
              <Input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="Optional code" />
            </div>
            <div>
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Delete Religion?</DialogTitle></DialogHeader>
