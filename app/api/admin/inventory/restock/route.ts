...
// POST: Restock (increment stock count for consumables/saleables)
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { error: txError } = await supabase.from("inventory_transactions").insert({
      ...
      quantity,
      notes: notes || `Restocked ${quantity} x ${item.name}`,
    });
    ...
    return errorResponse(error.message || "Failed to restock", 500);
  }
}
