...
interface ParentStatementResponse {
  children: Array<{ id: string; first_name: string; last_name: string; student_id: string }>;
  summary: {
    totalDue: number;
    totalPaid: number;
...
function BillCard({
  ...
  bill: ParentStatementResponse["bills"][number];
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex flex-wrap gap-3 text-xs">
          <span className="text-gray-400">
            Total: <strong className="text-gray-600">{formatMoney(bill.total_amount)}</strong>
          </span>
          <span className="text-gray-400">
            Paid: <strong className="text-emerald-600">{formatMoney(bill.amount_paid)}</strong>
          </span>
        </div>
...
function ChildrenSummary({ children: childrenList }: { children: ParentStatementResponse["children"] }) {
  ...
  return (
    <div className="rounded-xl border border-indigo-100 bg-indigo-50/40 p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-medium text-indigo-700">Children</span>
      </div>
      <div className="flex flex-wrap gap-2">
        {childrenList.map((child) => (
          <span
            key={child.id}
            className="inline-flex items-center gap-1.5 text-xs bg-white border border-indigo-100 rounded-full px-3 py-1 text-indigo-700"
          >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4">
      ...
      <p className="text-sm text-gray-500 text-center max-w-sm mb-6">
        None of your children have any bills yet. Check back later or
        contact the school finance office if you believe this is an error.
      </p>
      {onRefresh && (
...
export default function ParentFinancePage() {
  ...
  const [statement, setStatement] = useState<ParentStatementResponse | null>(null);
  const [refreshing, setRefreshing] = useState(false);

  const fetchStatement = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/parent/finance/statement");
      const payload = await res.json();
      if (!res.ok || !payload.success) {
        throw new Error(payload.error || "Failed to fetch statement");
      }
      setStatement(payload.data as ParentStatementResponse);
    } catch (error: unknown) {
      toast.error(
  ...
  const handlePayNow = useCallback(async (billId: string, amount: number) => {
    try {
      const callbackUrl = `${window.location.origin}/parent/finance`;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-gray-500">
              Pay and track billing for your children.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            {statement.children && statement.children.length > 0 && (
              <ChildrenSummary children={statement.children} />
            )}
