...
type QuestionBankQuestionsProps = {
  role: 'teacher' | 'admin';
};

// ─── Main component ───────────────────────────────────────────────────────────

export function QuestionBankQuestions({ role }: QuestionBankQuestionsProps) {
  ...
  async function handleSubmit() {
    // Check editability
    if (role === 'teacher' && bank && bank.created_by_teacher_id !== teacherId) {
    ...
    try {
      ...
      const url = isEditingMode
        ? `${apiPrefix}/questions/${questionId}`
        : `${apiPrefix}/banks/${bankId}/questions`;
      ...
      router.push(`${routePrefix}/${bankId}`);
    } catch (error) {
      console.error(error);
  ...
  const isEditable =
    role === 'admin' || (bank ? bank.created_by_teacher_id === teacherId : false);
        ...
        <Card className="w-full max-w-sm text-center shadow-sm">
          <CardContent className="pt-10 pb-8 px-8 space-y-5">
            ...
            <div className="space-y-1">
              <CardTitle className="text-base font-semibold text-gray-900">Bank not found</CardTitle>
    ...
    <div className="mx-auto max-w-3xl w-full space-y-8 pb-20 px-4 sm:px-6">
      ...
      <div className="space-y-5 border-t border-gray-100 pt-7">
        ...
        <div className="space-y-2">
          ...
          {questionImagePreview ? (
            ...
            <label
              className={`flex flex-col items-center justify-center gap-2 h-24 rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 cursor-pointer hover:bg-gray-100 hover:border-gray-300 transition-colors ${
      ...
      <div className="space-y-5 border-t border-gray-100 pt-7">
        <div className="flex items-center justify-between">
          <SectionLabel>
            3 — {questionType === 'objective' ? 'Options' : 'Model answer'}
          </SectionLabel>
            ...
            {Array.from({ length: optionCount }).map((_, idx) => {
              ...
              return (
                ...
                >
                  {/* Option label badge */}
                  <div
                    className={`mt-0.5 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-md text-xs font-bold transition-colors ${
                      isCorrect
                  ...
                  <button
                    ...
                    className={`flex-shrink-0 flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium transition-all ${
                      isCorrect
                        ? 'bg-emerald-600 text-white'
