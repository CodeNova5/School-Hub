...
async function createSplit(name, type, subaccounts) {
  try {
    ...
    // Transform subaccounts to use 'subaccount' key instead of 'subaccount_code'
    const transformedSubaccounts = subaccounts.map(acc => ({
      subaccount: acc.subaccount_code || acc.subaccount,
      share: acc.share,
    }));

    const payload = { name, type, currency: 'NGN', subaccounts: transformedSubaccounts };
