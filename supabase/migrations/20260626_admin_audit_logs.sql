-- =============================================================================
-- Admin Audit Logs — Database Trigger–Based Change Tracking
--
-- Captures INSERT / UPDATE / DELETE on every admin-managed table so that
-- school administrators can review exactly who changed what and when.
-- Old and new row values are stored as JSONB so the UI can show diffs.
-- =============================================================================
...
BEGIN
  ...
  -- If school_id is still null (e.g. super_admin operation on admins table),
  -- fall back to the caller's school context
...
-- ---------------------------------------------------------------------------
-- 4. Attach triggers to every admin-managed table
--    Each trigger fires AFTER INSERT / UPDATE / DELETE so the row is
--    already committed and we capture the final state.
-- ---------------------------------------------------------------------------
...
-- Student-guardian links (linking/unlinking parents to students)
...
-- The trigger function runs as the table owner, bypassing RLS on insert.
