...
-- -----------------------------------------------------------------------------
-- 1b) AI assistant daily token usage
-- -----------------------------------------------------------------------------
...
-- AI assistant daily usage helper functions
...
BEGIN
  ...
  WHERE user_id = p_user_id AND school_id = p_school_id AND usage_date = p_usage_date
  LIMIT 1;
...
BEGIN
  ...
  IF NOT FOUND THEN
    INSERT INTO ai_assistant_daily_usage (user_id, school_id, role, usage_date, tokens_used, quota_limit)
    VALUES (p_user_id, p_school_id, p_role, p_usage_date, 0, p_quota_limit)
  ...
  SET tokens_used = v_total, quota_limit = p_quota_limit, updated_at = now()
  WHERE id = v_row.id
  RETURNING * INTO v_row;
...
BEGIN
  ...
  WITH upsert AS (
    INSERT INTO ai_assistant_daily_usage (user_id, school_id, role, usage_date, tokens_used, quota_limit, created_at, updated_at)
    VALUES (p_user_id, p_school_id, p_role, p_usage_date, GREATEST(0, v_tokens_delta), p_quota_limit, now(), now())
...
-- WhatsApp logs updated_at trigger
DROP TRIGGER IF EXISTS trigger_update_whatsapp_logs_timestamp ON whatsapp_logs;
CREATE TRIGGER trigger_update_whatsapp_logs_timestamp
  BEFORE UPDATE ON whatsapp_logs
  FOR EACH ROW
  EXECUTE FUNCTION update_platform_timestamp();

-- AI assistant daily usage updated_at trigger
DROP TRIGGER IF EXISTS set_ai_assistant_daily_usage_updated_at ON ai_assistant_daily_usage;
CREATE TRIGGER set_ai_assistant_daily_usage_updated_at
  BEFORE UPDATE ON ai_assistant_daily_usage
  FOR EACH ROW
  EXECUTE FUNCTION update_platform_timestamp();
...
-- WhatsApp logs RLS
DROP POLICY IF EXISTS "Service role can manage all whatsapp logs" ON whatsapp_logs;
...
DROP POLICY IF EXISTS "Admins can manage their school whatsapp logs" ON whatsapp_logs;
CREATE POLICY "Admins can manage their school whatsapp logs"
  ON whatsapp_logs FOR ALL
  TO authenticated
  USING (
    school_id = (SELECT school_id FROM admins WHERE user_id = auth.uid())
  );

-- AI assistant daily usage RLS
DROP POLICY IF EXISTS "Users can view their own AI usage" ON ai_assistant_daily_usage;
CREATE POLICY "Users can view their own AI usage"
  ON ai_assistant_daily_usage FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can insert their own AI usage" ON ai_assistant_daily_usage;
CREATE POLICY "Users can insert their own AI usage"
  ON ai_assistant_daily_usage FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can update their own AI usage" ON ai_assistant_daily_usage;
CREATE POLICY "Users can update their own AI usage"
  ON ai_assistant_daily_usage FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid());
