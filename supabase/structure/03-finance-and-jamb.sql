...
-- Payroll updated_at triggers (reuses update_finance_updated_at defined below)
...
CREATE POLICY "School users can read payroll settings"
  ...
  TO authenticated
  USING (is_super_admin() OR school_id = get_my_school_id() OR teacher_id IN (SELECT id FROM teachers WHERE user_id = auth.uid()));
